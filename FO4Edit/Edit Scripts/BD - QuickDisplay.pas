...
const
  ...
  val1 = 'EDID';
  val2 = 'RNAM';
  val3 = 'Male Biped Model\MOD2'; 
  val4 = 'Female Biped Model\MOD3';
  val5 = 'BOD2\First Person Flags'; 
...
begin
  ...
  if lstPath.Count = 0 then 
    ...
    ed.top := TEdit(lstPath[lstPath.Count-1]).top + TEdit(lstPath[lstPath.Count-1]).height;
  //ed.Top := 50 + (30 * lstPath.Count);
  ed.Width := 400;
  
  cb := TComboBox.Create(frm);
  cb.Parent := frm;
  cb.Left := ed.Left + ed.Width + 10;
  cb.Top := ed.top;
  cb.Width := 150;
...
begin
  ...
  try
    ...
    lbl2.Top := btnPlus.top + btnPlus.height+5;
    lbl2.Left := 8;
    lbl2.height := 20;
    lbl2.AutoSize := False;
    lbl2.Wordwrap := False;
    ...
    cb1.Top := lbl2.top+lbl2.height+20;
    cb1.Left := 8;
    cb1.Width := 150;
    ...
    btnOk.Left := 120;
    btnOk.Top := cb2.top+cb2.height+10;
    ...
    if val5 <> '' then listsize := 4 else if val4 <> '' then listsize := 3 else listsize := 2;
    for i := 0 to listsize do begin
      AddPathEntry;
      if (i = 0) and (val1 <> '') then TEdit(lstPath[i]).Text := val1;
      if (i = 1) and (val2 <> '') then TEdit(lstPath[i]).Text := val2;
      if (i = 2) and (val3 <> '') then TEdit(lstPath[i]).Text := val3;
      if (i = 3) and (val4 <> '') then TEdit(lstPath[i]).Text := val4;
      if (i = 4) and (val5 <> '') then TEdit(lstPath[i]).Text := val5;
    end;
